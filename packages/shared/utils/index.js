 ...
 */
export function generateMockSignal(efficiency) {
    const size = 1024;
 ...
 */
export function calculateCoherence(magnitude) {
 ...
 */
export function calculateEntropy(magnitude) {
    if (!magnitude || magnitude.length === 0)
        return 0;
    const sum = magnitude.reduce((a, b) => a + b, 0);
    if (sum === 0)
 ...
 */
export function findDominantFrequency(magnitude, sampleRate = 1000) {
    if (!magnitude || magnitude.length === 0)
        return 0;
    let maxIndex = 0;
 ...
 */
export function extractHarmonics(magnitude, dominantHz, sampleRate = 1000) {
    const harmonics = [];
 ...
 */
export function getLiquidityRank(totalLiquidity) {
    if (totalLiquidity >= 10000000)
        return 'S';
    if (totalLiquidity >= 5000000)
        return 'A';
