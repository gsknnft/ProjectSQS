/**
 * TraditionalVsQuantum - Side-by-side comparison
 * 
 * Shows traditional technical analysis vs quantum signal analysis
 * This is the "WOW" moment - showing what traditional methods miss
 */
...
export const TraditionalVsQuantum: React.FC<TraditionalVsQuantumProps> = ({
  ...
  const drawTraditionalChart = () => {
    ...
    const traditionalAnomalies = anomalies.filter(a => a.type === 'traditional');
  ...
  const drawQuantumChart = () => {
    ...
    const quantumAnomalies = anomalies.filter(a => a.type === 'quantum');
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
        ...
        <div className="mt-4 space-y-2">
          ...
          {anomalies.filter(a => a.type === 'traditional').slice(0, 3).map((anomaly, i) => (
      ...
      <div className="bg-black border border-green-500 rounded-lg p-4">
        ...
        <div className="mt-4 space-y-2">
          ...
          {anomalies.filter(a => a.type === 'quantum').slice(0, 3).map((anomaly, i) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-800 p-4 rounded">
            <div className="text-2xl font-bold text-green-400">
              {anomalies.filter(a => a.type === 'quantum').length - anomalies.filter(a => a.type === 'traditional').length}
            </div>
            <div className="text-sm text-gray-400">More Anomalies Detected</div>
          </div>
          <div className="bg-gray-800 p-4 rounded">
            <div className="text-2xl font-bold text-green-400">
              {Math.round((signalData.coherence.filter(c => c > 0.7).length / signalData.coherence.length) * 100)}%
            </div>
            <div className="text-sm text-gray-400">Field Coherence</div>
          </div>
          <div className="bg-gray-800 p-4 rounded">
            <div className="text-2xl font-bold text-green-400">
              {signalData.fieldState.filter(s => s === 'coherent').length}
            </div>
